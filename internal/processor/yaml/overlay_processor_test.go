...
var mcpContent = testutils.LoadFixture("testdata/mcp_server_card.yaml")
var mcpDoc = testutils.UnmarshalFixture[map[string]any]("testdata/mcp_server_card.yaml")
...
func TestApply_OutputFields_OriginalDefinitionFieldsPreserved(t *testing.T) {
	...
	if rd.URL != "https://example.com/mcp-server-card.yaml" {
...
func TestApply_Update_NestedObject_ReplacesNode(t *testing.T) {
	...
	// The old key not in Data must be gone after update.
...
func TestApply_Remove_NilData_RemovesTargetedNode(t *testing.T) {
	// nil Data is fast-pathed (returns original patch), so the selector itself
	// is resolved and the matched node is deleted.
	p := newProcessor(t, mcpContent)
	result := testutils.ApplyAndParse(t, p, model.OverlayDefinition{
		Overlay: model.Overlay{Patches: []model.Patch{
			{
	...
	if _, exists := result["_meta"]; exists {
		t.Error("_meta should have been removed when Data is nil")
	}
}
...
func TestApply_MixedActions_AppliedInOrder(t *testing.T) {
	...
	// because Decompose resolves selectors against the live content at that point.
	p := newProcessor(t, mcpContent)
	result := testutils.ApplyAndParse(t, p, model.OverlayDefinition{
		Overlay: model.Overlay{Patches: []model.Patch{
...
func TestApply_JSONPath_NotFound_Merge_ReturnsError(t *testing.T) {
...
func TestApply_JSONPath_NotFound_Update_ReturnsError(t *testing.T) {
