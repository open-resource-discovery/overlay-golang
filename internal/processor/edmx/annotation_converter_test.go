...
func TestConvert_LineItemRecordWithTypeAndPath_ProducesTypedRecordAndPathExpression(t *testing.T) {
	assertXML(t,
		AnnotationConverter(0).Convert("@UI.LineItem", []any{
			map[string]any{
				"$Type": "UI.DataField",
				"Value": map[string]any{"$Path": "title"},
			},
		}),
		`
